...
(function () {
  ...
  function log(..._args) {
    if (DEBUG) console.log("[NAV]", ..._args);
  }
  ...
  const canonicalItems = Array.from(
    nav.querySelectorAll(":scope > a[data-nav-item]"),
  );
  ...
  // --- BREAKPOINT LOGGING ---
  // For each item, log the window width at which it wraps to the next line
  function logNavBreakpoints() {
    restoreAll();
    const itemBreakpoints = [];
    canonicalItems.forEach((item, idx) => {
      // Binary search for the width at which this item wraps
      let min = 300,
        max = 2000,
        found = null;
      ...
      while (max - min > 2) {
        ...
        window.resizeTo(mid, window.innerHeight);
        restoreAll();
        // If this item is on a new line, record
        const rect = item.getBoundingClientRect();
        const navRect = nav.getBoundingClientRect();
        if (rect.top > navRect.top + 2) {
          max = mid;
          found = mid;
        } else {
          min = mid;
      ...
      itemBreakpoints.push({
        key: item.getAttribute("data-nav-item"),
        breakpoint: found,
      });
      item.style.display = originalDisplay;
    });
    console.log("NAV BREAKPOINTS:", itemBreakpoints);
  }
  // Uncomment to run on load:
  window.addEventListener("load", logNavBreakpoints);

  // Insert missing helper functions used later
  function snapshotItemWidths(list) {
    return list.map((a) => ({
      key: a.getAttribute("data-nav-item"),
  ...
  function getAvailableNavWidth() {
    ...
    const gapPx =
      parseFloat(
        getComputedStyle(header).columnGap ||
          getComputedStyle(header).gap ||
          "0",
      ) || 0; // grid gap
    ...
    const mobileW = outerWidth(
      mobileToggle && mobileToggle.offsetParent ? mobileToggle : null,
    ); // hidden on desktop => width 0
    ...
    const occupied =
      logoW + langW + mobileW + gapsTotal + paddingLeft + paddingRight;
    const available = headerRect.width - occupied;
    log(
      "Widths header",
      headerRect.width.toFixed(1),
      "occupied",
      occupied.toFixed(1),
      "available",
      available.toFixed(1),
    ...
    canonicalItems.forEach((a) => {
      if (keySet.has(a.getAttribute("data-nav-item"))) {
        ...
        if (moreDropdown.firstChild)
          moreDropdown.insertBefore(a, moreDropdown.firstChild);
        else moreDropdown.appendChild(a);
      }
    });
  ...
  function collapseIfNeeded() {
    if (collapsing) {
      log("Skip: collapsing already running");
      return;
    }
    collapsing = true;
    ...
    if (isMobile) {
      restoreAll();
      lastLayout = { available: 0, requiredAll: 0, dropdownKeys: [] };
      collapsing = false;
      return;
    }
    if (moreDropdown.classList.contains("open")) {
      log("Dropdown open: skip");
      collapsing = false;
      return;
    }
    mo.disconnect();
    ...
    log(
      ...
      snapshotItemWidths(canonicalItems),
    );
    if (!canonicalItems.length) {
      log("No items after restore");
    ...
    const gap =
      parseFloat(
        getComputedStyle(nav).columnGap || getComputedStyle(nav).gap || "0",
      ) || 0;
    const itemsWidth = canonicalItems.reduce(
      (s, a) => s + a.getBoundingClientRect().width,
      0,
    );
    const requiredAll =
      itemsWidth + gap * Math.max(0, canonicalItems.length - 1);
    ...
    const tol = 2; // px
    if (
      lastLayout.dropdownKeys.length &&
      Math.abs(lastLayout.available - available) <= tol &&
      Math.abs(lastLayout.requiredAll - requiredAll) <= tol
    ) {
    ...
    while (
      required > available &&
      nav.querySelector(":scope > a[data-nav-item]")
    ) {
      ...
      else moreDropdown.appendChild(last);
    ...
    lastLayout = {
      available,
      requiredAll,
      dropdownKeys: Array.from(
        moreDropdown.querySelectorAll("a[data-nav-item]"),
  ...
  if (moreToggle) {
    ...
    function getDropdownItems() {
      return Array.from(moreDropdown.querySelectorAll("a[data-nav-item]"));
    }
    ...
    moreToggle.addEventListener("click", (e) => {
      ...
      if (open && e.detail === 0) {
        // keyboard-triggered (Enter/Space dispatch click with detail 0)
    ...
    moreDropdown.addEventListener("keydown", (e) => {
      ...
      switch (e.key) {
        case "ArrowDown": // cycle forward
          ...
          else items[(currentIndex + 1) % items.length].focus();
          break;
