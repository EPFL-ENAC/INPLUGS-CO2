...
export class WebmanifestGenerator {
  ...
  async generateWebManifests(routesConfig, localeData) {
    ...
    for (const locale of this.locales) {
      ...
      const localizedName = translator('site.name') || translator('meta.title') || baseManifest.name || 'INPLUGS'
      const localizedShortName = translator('site.short_name') || translator('meta.short_title') || baseManifest.short_name || 'INPLUGS'
      const localizedDescription = translator('site.description') || translator('meta.description') || baseManifest.description || 'INPLUGS CO2 Calculator'
