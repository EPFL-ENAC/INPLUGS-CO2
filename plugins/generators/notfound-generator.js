...
import { getRoutePath, loadLocaleData, makeTranslator } from '../utils/locale-utils.js'
...
export class NotFoundGenerator {
  ...
  async write404s(routesConfig) {
    for (const locale of this.locales) {
      ...
      if (existsSync(custom404LocalePath) || existsSync(custom404Path)) {
        html = await this.renderCustom404(locale, custom404Path, custom404LocalePath, routesConfig)
      }
  ...
  async renderCustom404(locale, custom404Path, custom404LocalePath, routesConfig) {
    if (!this.env) {
      console.warn('Nunjucks environment not available for custom 404 rendering')
      return null
    }
    ...
    try {
      ...
      const html = this.env.render('pages/' + templateFile, {
        ...
        rtl: this.localesMeta[locale]?.rtl || ['ar','he','fa','ur'].includes(locale),
        t: translator,
        ...Object.fromEntries(
          Object.entries(localeData[locale] || localeData[this.defaultLocale] || {})
        ),
  ...
  generateBasic404(locale, meta, routesConfig) {
    ...
    for (const altLocale of this.locales) {
      const altUrl = getRoutePath('404', altLocale, routesConfig) || `/${altLocale}/404`
      alternateUrls[altLocale] = altUrl
    }
    
    const canonicalUrl = getRoutePath('404', locale, routesConfig) || `/${locale}/404`
    ...
    return this.templateEnv.render('404.html.njk', {
      ...
      description: translator('404.description') || 'The page you are looking for could not be found.',
      message: translator('404.message') || 'The page you are looking for could not be found.',
      canonicalUrl,
      alternateUrls,
      errorCode: '404',
